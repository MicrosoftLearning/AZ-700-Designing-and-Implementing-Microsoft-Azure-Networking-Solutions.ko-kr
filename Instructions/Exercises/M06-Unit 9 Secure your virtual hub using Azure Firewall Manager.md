...
1. **리소스 그룹**에서 **새로 만들기**를 선택하고, 이름으로 **fw-manager-rg**를 입력하고, **확인**을 선택합니다.

1. **이름**에 **Spoke-01**을 입력합니다.
...
1. 여기에 나열된 다른 주소 공간(예: **10.1.0.0/16**)을 **삭제**합니다.

1. **서브넷 이름** 아래에서 **기본값**이라는 단어를 선택합니다.

1. **서브넷 편집** 대화 상자에서 이름을 **Workload-01-SN**으로 변경합니다.

1. **서브넷 주소 범위**를 **10.0.1.0/24**로 변경합니다.
...
1. 검색 상자에 **firewall manager**를 입력하고 **Firewall Manager**가 표시되면 선택합니다.

1. **Firewall Manager** 페이지의 개요 페이지에서 **보안 가상 허브 보기**를 선택합니다.

1. **가상 허브** 페이지에서 **새 보안 가상 허브 만들기**를 선택합니다.
...
1. **허브 주소 공간**에 **10.2.0.0/16**을 입력합니다.

1. **새 vWAN**을 선택합니다.

1. **Virtual WAN 이름**에 **Vwan-01**을 입력합니다.

1. **다음: Azure Firewall**을 선택합니다.
    ![새 보안 가상 허브 만들기 - 기본 사항 탭](../media/create-new-secured-virtual-hub-1.png)

1. **다음: 보안 파트너 공급자**를 선택합니다.
...
1. 배포가 완료되면 Azure Portal 홈페이지에서 **모든 서비스**를 선택합니다.
...
1. 나중에 사용할 공용 IP 주소(예: **51.143.226.18**)를 적어 둡니다.
...
1. **연결 이름**에 **hub-spoke-01**을 입력합니다.
...
1. 위의 4~9단계를 반복하여 다른 유사한 연결을 만들되, **hub-spoke-02**라는 연결 이름을 사용하여 **Spoke-02** 가상 네트워크에 연결합니다.
