...
위의 1~14단계를 반복하여 유사하지만 다음 정보를 사용하여 다른 가상 네트워크 및 서브넷을 만듭니다. 첫 번째 가상 네트워크가 배포를 완료할 때까지 기다릴 필요가 없습니다. 
...
2. 
1. 포털에서 **Vwan-01** Virtual WAN을 검색하여 선택합니다.
...
1. **Srv-workload-02**의 **개요** 페이지의 오른쪽 창에서 **네트워킹** 섹션 아래에 있는 **개인 IP 주소**(예: **10.1.0.4**)를 적어둡니다.
...
1. **Firewall Manager** 블레이드에서 **Azure Firewall 정책**을 선택합니다.
...
1. **규칙 컬렉션 추가** 페이지의 **이름**에서 `App-RC-01`을 입력합니다.
...
1. **규칙**의 **이름**에 `Allow-msft`를 입력합니다.

1. **원본 유형**에 대해 **IP 주소**를 선택합니다.

1. **원본**에 대해 *를 입력합니다.

1. **프로토콜**로 `http,https`를 입력합니다.

1. **대상 유형**이 **FQDN**인지 확인합니다.

1. **대상**으로 `*.microsoft.com`을 입력합니다.
...
1. **대상 포트**로 `3389`를 입력합니다.

1. **대상 IP 주소**로 앞에서 적어 둔 방화벽 가상 허브 공용 IP 주소(예: **51.143.226.18**)를 입력합니다.

1. **변환된 형식**으로 **IP 주소**를 선택합니다.
...
1. **규칙**의 **이름**에 `Allow-vnet`를 입력합니다.
...
1. **대상**으로 앞에서 적어 둔 **Srv-workload-02**의 개인 IP 주소(예: **10.1.0.4**)를 입력합니다.
