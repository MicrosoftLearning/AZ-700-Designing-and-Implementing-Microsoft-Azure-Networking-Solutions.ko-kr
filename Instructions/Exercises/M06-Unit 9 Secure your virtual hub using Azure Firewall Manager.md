...
# M06-Unit 9 Azure Firewall Manager를 사용하여 가상 허브 보안
...
1. Azure Portal 홈 페이지의 검색 상자에 가상 네트워크를 입력**하고 가상 네트워크가**** 나타나면 선택합니다**.
2. **만들기**를 실행합니다.
...
1. Azure Portal 홈페이지에서 모든 서비스를** 선택합니다**.

2. 검색 상자에 **firewall manager**를 입력하고 **Firewall Manager**가 표시되면 선택합니다.

3. **방화벽 관리자** 페이지의 개요 페이지에서 보안 가상 허브** 보기를 선택합니다**.

4. **가상 허브** 페이지에서 **새 보안 가상 허브 만들기**를 선택합니다.

5. 리소스 그룹의 경우 **fw-manager-rg**를 선택합니다**.**

6. **지역**에서 지역을 선택합니다.

7. **보안 가상 허브 이름으로** Hub-01**을 입력합니다**.
...
7. 리소스 그룹의 경우 **fw-manager-rg**를 선택합니다**.**
8. 가상 네트워크의 경우 **Spoke-01**을 선택합니다**.**
9. **만들기**를 실행합니다.
...
1. Cloud Shell 창의 도구 모음에서 파일 업로드/다운로드 아이콘을 선택하고 **드롭다운 메뉴에서 다음 파일 업로드**를 선택하고 **다음 파일 **FirewallManager.json** 및 **FirewallManager.parameters.json**을 원본 폴더 **F:\Allfiles\Exercises\M06**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드합니다.**
...
1. **Firewall Manager**의 개요 페이지에서 Azure Firewall 정책** 보기를 선택합니다**.

1. Azure Firewall 정책** 만들기를 선택합니다**.

1. **리소스 그룹에서** fw-manager-rg**를 선택합니다**.

5. **정책 세부 정보**에서 **이름**에 대해 **Policy-01**을 입력합니다.

1. **지역에서** 지역을 선택합니다.

1. 정책 계층**에서 **표준을** 선택합니다**.
...
1. 프로토콜의 경우 **TCP**를 선택합니다**.**

1. **대상 포트**에 대해 **3389**를 입력합니다.

1. 대상 유형**에 대해 **IP 주소를** 선택합니다**.
...
1. 규칙 컬렉션 형식**의 경우 **네트워크를** 선택합니다**.

1. **우선 순위**에 대해 **100**을 입력합니다.

1. 규칙 컬렉션 작업의 경우 **[허용 **]을 선택합니다**.**
...
1. Azure Portal 홈페이지에서 **Firewall Manager**를 선택합니다.
   - Firewall Manager 아이콘이 홈페이지에 표시되지 않으면 **모든 서비스**를 선택합니다. 그런 다음 검색 상자에 **firewall manager**를 입력하고 **Firewall Manager**가 표시되면 선택합니다.
1. **Firewall Manager**의 보안** 아래에서 **Azure Firewall 정책을** 선택합니다**.
...
1. 방화벽 관리자**에서 **가상 허브를** 선택합니다**.
1. 허브-01을** 선택합니다**.
1. 설정** 보안 **구성**을 선택합니다**.
1. 인터넷 트래픽에서 **Azure Firewall**을 선택합니다**.**
1. 프라이빗 트래픽에서 **Azure Firewall**을 통해 보내기를 선택합니다**.**
1. **저장**을 선택합니다. 
...
1. 컴퓨터 상자에서 **방화벽의 공용 IP 주소 **(예: **51.143.226.18**)를 입력**합니다.**

1. **옵션 표시**를 선택합니다.

1. **사용자 이름** 상자에 TestUser를 입력**합니다**.

1. **연결**을 선택합니다.

   ![RDP를 srv-workload-01에 연결](../media/rdp-srv-workload-01.png)

1. **자격 증명** 입력 대화 상자에서 배포 중에 제공한 암호를 사용하여 Srv-workload-01** 서버 가상 머신에 로그인**합니다.
...
1. 컴퓨터 상자에서 **Srv-workload-02**의 **개인 IP 주소 **(예: **10.1.1.4**)를 입력 **** 합니다.

1. **자격 증명** 입력 대화 상자에서 **사용자 이름 **TestUser** 및 배포 중에 제공한 암호를 사용하여 Srv-workload-02** 서버에 로그인합니다.
