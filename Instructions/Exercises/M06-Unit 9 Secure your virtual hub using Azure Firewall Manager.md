...
1. Cloud Shell 창의 도구 모음에서 **파일 업로드/다운로드** 아이콘을 선택하고 드롭다운 메뉴에서 **업로드**를 선택하고 다음 파일 **FirewallManager.json** 및 **FirewallManager.parameters.json**을 원본 폴더 **F:\Allfiles\Exercises\M06**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드합니다.
...
1. **방화벽 관리자**의 개요 페이지에서 **Azure Firewall 정책 보기를** 선택합니다.

1. **Azure Firewall 정책 만들기**를 선택합니다.

1. **리소스 그룹에서** **fw-manager-rg를** 선택합니다.
...
1. Azure Portal 홈페이지에서 **Firewall Manager**를 선택합니다.
   - Firewall Manager 아이콘이 홈페이지에 표시되지 않으면 **모든 서비스**를 선택합니다. 그런 다음 검색 상자에 **firewall manager**를 입력하고 **Firewall Manager**가 표시되면 선택합니다.
1. **방화벽 관리자**의 **보안**에서 **Azure Firewall 정책을** 선택합니다.
...
1. **인터넷 트래픽**에서 **Azure Firewall** 선택합니다.
1. **프라이빗 트래픽**에서 **Azure Firewall 통해 보내기**를 선택합니다.
1. **저장**을 선택합니다. 
1. 이 설정은 완료하는 데 몇 분 정도 걸립니다.
...
1. **자격 증명 입력** 대화 상자에서 배포 중에 제공한 암호를 사용하여 **Srv-workload-01** 서버 가상 머신에 로그인합니다.
