...
---
# M01-Unit 8 커넥트 글로벌 가상 네트워크 피어링을 사용하여 두 Azure Virtual Network
...
1. Cloud Shell 창의 도구 모음에서 파일 업로드/다운로드** 아이콘을 선택하고 **드롭다운 메뉴에서 **다음** 파일 ManufacturingVMazuredeploy.json** 및 **ManufacturingVMazuredeploy.parameters.json** 파일을 **원본 폴더 **F:\Allfiles\Exercises\M01**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드합니다.
...
1. ManufacturingVM | ** 커넥트RDP 파일을** 다운로드합니다.

1. RDP 파일을 데스크톱에 저장합니다.

1. RDP 파일 및 사용자 이름을 ** 사용하여 ManufacturingVM에 커넥트TestUser** 및 배포 중에 제공한 암호입니다.

1. Azure Portal 홈페이지에서 Virtual Machines**를 선택합니다**.

1. **TestVM1**을 선택합니다.

1. TestVM1에서 RDP**를 커넥트 &gt; 선택합니다**.

1. TestVM1 | ** 커넥트RDP 파일을** 다운로드합니다.
...
1. Azure Portal에서 Cloud Shell 창 내에서 **PowerShell** 세션을 엽니다**.** 필요한 경우 기본 설정을 사용하여 Cloud Shell 스토리지를 만듭니다.
