...
>**참고**: UI에 서브넷을 추가하는 옵션이 없는 경우 단계를 완료하고 게이트웨이를 만든 후 백 엔드 서브넷을 추가합니다. 
...
1. 열리는 **백 엔드 풀 추가** 창에서 다음 값을 입력하여 빈 백 엔드 풀을 만듭니다.
...
1. **백 엔드 풀 추가 창에서** **추가**를 선택하여 백 엔드 풀 구성을 저장하고 **백 엔드** 탭으로 돌아갑니다.
...
1. **규칙 이름** 상자에 **RoutingRule을 입력합니다**.
...
1. Cloud Shell 창의 도구 모음에서 **파일 업로드/다운로드** 아이콘을 선택하고 드롭다운 메뉴에서 **업로드**를 선택하고 **다음 파일 backend.json** 및 **backend.parameters.json**을 원본 폴더 **F:\Allfiles\Exercises\M05**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드합니다.
