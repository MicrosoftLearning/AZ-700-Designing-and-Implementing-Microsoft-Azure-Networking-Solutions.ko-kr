...
1. IP 주소 탭의 **IPv4 주소 공간** 상자에 10.1.0.0/16**을 입력**** 합니다.**
...
1. **부하 분산 장치의** 백 엔드 풀 페이지에서 설정** 상태 **프로브를** 선택한 **다음 추가**를 선택합니다**.
...
1. Cloud Shell 창의 도구 모음에서 파일 업로드/다운로드** 아이콘을 선택하고 **드롭다운 메뉴에서 **다음 파일 azuredeploy.json 및 **azuredeploy.parameters.json**** 파일을 **원본 폴더 **F:\Allfiles\Exercises\M08**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드**합니다.
...
1. Azure 홈페이지에서 전역 검색을 사용하여 Virtual Machines**를 입력**하고 서비스에서 가상 머신을 선택합니다. 
...
1. 홈을 선택한 **다음, Azure Portal 홈 페이지에서 모든 리소스를** 선택한 **다음, 방금 만든 myTestVM** 가상 머신을 선택합니다**.**

1. **개요** 페이지에서 **연결**을 선택한 다음, **Bastion**을 선택합니다.

1. 배스천** 사용을 선택합니다**.

1. **사용자 이름** 상자에 TestUser**를 입력**하고 **암호** 상자에 배포 중에 제공한 암호를 입력한 다음, 커넥트** 선택합니다**.

1. **myTestVM** 창이 다른 브라우저 탭에서 열립니다.

1. 네트워크** 창이 **나타나면 예를** 선택합니다**.

1. 작업 표시 **줄에서 Internet Explorer** 아이콘을 선택하여 웹 브라우저를 엽니다.

1. Internet Explorer 11** 설정 대화 상자에서 **확인을** 선택합니다**.
...
1. Azure Portal 홈페이지에서 모든 서비스를 선택한 **다음 페이지 위쪽의 검색 상자에 Log Analytics**를 입력**하고 필터링된 목록에서 Log Analytics** 작업 영역을 선택합니다**.**
...
1. 페이지의 오른쪽 위 모서리에서 X를 선택하여 지금은 메트릭 창을 닫습니다****.**** 잠시 후 창을 다시 열 것입니다.
...
1. 를 다운로드하려면 토폴로지 다이어그램의 SVG 파일 복사본을 선택하고, 토폴로지** 다운로드를 선택하고**, 다운로드 폴더에 **파일을 저장합니다**. 

1. 오른쪽 위 모서리에서 메트릭** 보기를 선택하여 **화면 오른쪽의 메트릭 창을 다시 엽니다.
...
1. 흐름 배포 탭을 **선택하고 페이지를 아래로 스크롤하여 VM 흐름 만들기 및 네트워크 트래픽** 섹션 아래의 **차트를 확인**합니다. 
...
1. **Service Health&gt;Service 문제** 페이지의 왼쪽 메뉴에서 Resource Health**를 선택합니다**.
...
1. 모니터링에서 **진단 설정을** 선택한 **다음 진단 설정** 추가를** 선택합니다**.

   ![진단 설정>진단 설정 추가 단추가 강조 표시된 화면](../media/diagnostic-settings-1.png)

1. **진단 설정** 페이지의 이름 상자에 myLBDiagnostics를 입력**합니다**.
