...
1. Azure Portal 홈페이지의 검색 상자에 DDoS를 입력**하고 DDoS** 보호 계획이** 나타나면 선택합니다**.

1. **+ 만들기**를 선택합니다.

1. **기본 사항** 탭의 **리소스 그룹** 목록에서 방금 만든 리소스 그룹을 선택합니다.

1. 인스턴스 이름** 상자에 MyDdoSProtectionPlan**을 입력**한 다음 검토 + 만들기**를 선택합니다**.**
...
1. Azure Portal 홈페이지에서 리소스 만들기를 선택한 **다음, 검색 상자에 Virtual Network를 입력**한 다음, 가상 네트워크가**** 나타나면 선택합니다**.**

1. **Virtual Network** 페이지에서 **생성**를 선택합니다.

1. **기본 사항** 탭에서 이전에 만든 리소스 그룹을 선택합니다.

1. 이름 상자에서 **MyVirtualNetwork**를 입력**한 다음 보안 탭을 **** 선택합니다.** 

1. 보안 탭의 **DDoS 네트워크 보호** 옆에 **있는 [사용]**을 선택합니다**.**
...
1. **이름** 상자에 MyPublicIPAddress**를 입력**합니다.

1. **IP 주소 할당** 아래에서 **정적**을 선택합니다.

1. DNS 이름 레이블에서 **mypublicdnsxx**를 입력**합니다(여기서 xx는 이니셜을 고유하게 만들기 위한 이니셜임**).

1. **만들기**를 실행합니다.

1. 원격 분석을 설정하려면 Azure 홈페이지로 이동하여 모든 리소스를** 선택합니다**.
...
1. 메트릭 상자에서 **DDoS**를 삭제한 인바운드 패킷을 선택합니다**.**
...
1. Azure Portal 홈페이지에서 리소스 만들기를 선택한 **다음, 검색 상자에 가상 머신을 입력**한 다음, 가상 머신****이 나타나면 선택합니다**.**
...
1. **기본 사항** 탭에서 아래 표의 정보를 사용하여 새 VM을 만듭니다.
   ...
   | 이미지                 | **Ubuntu Server 20.04 LTS - Gen 2** (필요한 경우 VM 생성 구성 링크 선택) |                     
...
1. **새 키 쌍** 생성 대화 상자에서 프라이빗 키 다운로드를 선택하고 **리소스**를 만듭니다.
...
1. DDoS 공격 아래를 선택하거나 신호 이름으로 선택 **하지 않습니다** .

1. 경고 논리에서 연산자** **설정을 찾고 보다 크거나 같음을** 선택합니다**.

1. 임계값**에 **1 **(공격 중인 수단)을 입력**합니다.

1. 세부 정보 탭으로 이동하고 경고 규칙 이름을 선택하고 **MyDdosAlert**를 입력**** 합니다.
...
1. Azure DDoS 시뮬레이션 테스트 정책 검토 [](https://learn.microsoft.com/azure/ddos-protection/test-through-simulations#azure-ddos-simulation-testing-policy)

1. 승인된 테스트 파트너를 사용하여 DDoS 테스트 공격을 구성합니다. BreakingPoint Cloud를 사용하여 테스트하는 경우 아래 스크린샷의 설정을 사용하지만(평가판 계정으로 100k pps 테스트 크기를 선택해야 할 수도 있음) 대상 IP 주소 상자(예: 51.140.137.219 **) ![DDOSAttack에서 **고유한 **MyPublicIPAddress** 리소스의 IP 주소를** 지정**합니다.](https://user-images.githubusercontent.com/46939028/138599420-58bef33a-2597-4fa2-919f-bf1614037bc3.JPG)
