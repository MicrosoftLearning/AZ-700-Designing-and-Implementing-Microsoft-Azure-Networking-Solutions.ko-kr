...
1. Cloud Shell 창의 도구 모음에서 **파일 관리** 아이콘을 선택하고 드롭다운 메뉴에서 **업로드**를 선택한 후 **firewall.json** 및 **firewall.parameters.json** 파일을 소스 폴더 **F:\Allfiles\Exercises\M06**에서 Cloud Shell 홈 디렉토리로 하나씩 업로드합니다.
...
1. Firewall Manager를 사용하지 않으므로 **방화벽 관리 NIC 사용**의 확인란을 선택 취소합니다. 
