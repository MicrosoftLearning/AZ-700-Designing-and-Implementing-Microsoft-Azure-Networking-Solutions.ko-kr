...
1. Azure Portal 홈페이지에서 `virtual network`를 검색한 다음, 결과에서 가상 네트워크**가상 네트워크**를 선택합니다.
...
1. **IP 주소** 탭을 선택하고 다음 값을 입력합니다(서브넷 이름을 변경하려면 **기본값** 선택). ![그래픽 사용자 인터페이스, 텍스트, 애플리케이션, 메일 자동 생성된 설명](../media/create-virtual-network-ip.png)
...
1. Azure Storage 서비스에 대해 아웃바운드 통신을 허용하는 규칙을 만듭니다. 다음 정보를 입력하거나 선택합니다. ![그래픽 사용자 인터페이스, 애플리케이션 자동 생성된 설명](../media/add-outbound-security-rule.png)
...
1. 인바운드 보안 규칙 추가에서 다음 값을 입력합니다. ![그래픽 사용자 인터페이스, 애플리케이션 자동 생성된 설명](../media/add-inbound-security-rule.png)
...
1. 스토리지 계정이 성공적으로 배포되면 **리소스로 이동**을 선택합니다. 

## 작업 7: 스토리지 계정에 파일 공유 만들기

1. 스토리지 계정의 **데이터 스토리지** 블레이드에서 **파일 공유**를 선택합니다.

1. **+ 파일 공유**를 선택합니다. 

1. **이름**에 **마케팅**을 입력하고 **다음: 백업**을 선택합니다.

1. 다음 그림과 같이 **백업 사용**을 선택 취소합니다. 
...
1. **공용 네트워크 액세스** 섹션에서 **관리**를 선택합니다.

1 **공용 액세스 범위** 섹션에서 **선택한 네트워크에서 사용**을 선택합니다.

1. **+ 기존 가상 네트워크 추가**를 선택한 다음, **기존 가상 네트워크**를 추가합니다. 
...
1. **Key1** 값으로 **Show**를 사용합니다. 나중에 필요하므로 이 값을 복사해 둡니다. 
...
1. Cloud Shell 창의 도구 모음에서 **파일 관리** 아이콘을 선택하고 드롭다운 메뉴에서 **업로드**를 선택한 후 **VMs.json** 및 **VMs.parameters.json** 파일을 Cloud Shell 홈 디렉터리에 업로드합니다. 이 모듈 07, 연습 05. 
...
1. 포털에서 **ContosoPrivate** 가상 머신을 검색하여 선택합니다.

1. **연결**을 선택하고, **연결**을 선택한 다음, **RDP 파일 다운로드**를 선택합니다. 메시지가 표시되면 다운로드를 확인합니다.

1. **다운로드** 폴더에서 ContosoPrivate.rdp 파일을 엽니다.

1. **연결**을 선택하고 가상 머신 암호를 제공합니다. **확인**을 선택하고 인증서 경고에 **예**를 표시합니다. 

1. PowerShell을 사용하여 파일 공유를 만듭니다. <storage-account-key1-value> , <storage-account-name>(즉, contosostoragexx)을 스토리지 계정을 만들 때의 값으로 바꿉니다. 
    ```powershell
    ...
    ```
1. VM에 아웃바운드 연결이 없는지 확인합니다. 프라이빗 서브넷에 연결된 네트워크 보안 그룹이 인터넷에 대한 아웃바운드 액세스를 허용하지 않아 응답이 수신되지 않습니다.
...
1. 스토리지 계정으로 이동하여 **파일 공유**를 선택한 다음, **마케팅** 파일 공유를 선택합니다. 

1. **찾아보기**를 선택하고 액세스 거부 오류를 확인합니다. 실제로 표시되는 오류는 다르게 보일 수 있습니다.  컴퓨터가 CoreServicesVNet 가상 네트워크의 프라이빗 서브넷에 없어 액세스가 거부되었습니다.
