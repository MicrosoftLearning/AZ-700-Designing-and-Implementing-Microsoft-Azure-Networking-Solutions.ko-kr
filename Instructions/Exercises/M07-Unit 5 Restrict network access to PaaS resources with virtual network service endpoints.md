...
# M07-Unit 5 가상 네트워크 서비스 엔드포인트를 사용하여 PaaS 리소스에 대한 네트워크 액세스 제한
...
![서비스 엔드포인트 아키텍처의 다이어그램.](../media/5-exercise-restrict-network-paas-resources-virtual-network-service-endpoints.png)
...
1. 다음 정보를 ![입력하거나 선택합니다. 그래픽 사용자 인터페이스, 텍스트, 애플리케이션 설명이 자동으로 생성됨](../media/create-virtual-network.png)
...
1. IP 주소 탭을 **** 선택하고 다음 값을 입력합니다(서브넷 이름을 변경하려면 기본값** 선택**): ![그래픽 사용자 인터페이스, 텍스트, 애플리케이션, 전자 메일 설명이 자동으로 생성됨](../media/create-virtual-network-ip.png)
...
1. 보안** 탭을 **선택하고 그래픽 사용자 인터페이스, 텍스트, 애플리케이션, 전자 메일 설명이 자동으로 생성되는 값을 ![입력합니다.](../media/ create-virtual-network-security.png)
...
1. 가상 네트워크에 서브넷을 추가합니다. 설정** 아래에서 **서브넷**을 선택한 **다음 그림과 같이 ![+ 서브넷**을 선택합니다**. 그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/create-subnet.png)
...
![그래픽 사용자 인터페이스, 텍스트, 애플리케이션, 메일 자동 생성된 설명](../media/configured-subnets.png)
...
1. Azure Storage 서비스에 대해 아웃바운드 통신을 허용하는 규칙을 만듭니다. 다음 정보를 ![입력하거나 선택합니다. 그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/add-outbound-security-rule.png)
...
1. 다음 정보를 ![입력하거나 선택합니다. 그래픽 사용자 인터페이스, 애플리케이션, 전자 메일 설명이 자동으로 생성됨](../media/add-outbound-security-rule-deny.png)
...
1. 인바운드 보안 규칙 추가에서 다음 값을 ![입력합니다. 그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/add-inbound-security-rule.png)
...
> **경고**: RDP 포트 3389가 인터넷에 노출됩니다. 이는 테스트용으로만 추천됩니다. 프로덕션 환경의 경우 VPN 또는 프라이빗 연결을 사용하는 것이 좋습니다.
...
1. Azure Portal에서 Storage 계정을 선택합니다.
...
1. 검토를 선택한 **다음 만들기**를 선택합니다**.**

## 작업 7: 스토리지 계정에 파일 공유 만들기

1. 스토리지 계정이 만들어진 후에 포털 맨 위에 있는 **리소스, 서비스 및 문서 검색** 상자에 스토리지 계정의 이름을 입력합니다. 스토리지 계정의 이름이 검색 결과에 표시되면 선택합니다.
1. 다음 그림![과 같이 파일 공유**를 선택합니다**. 그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/new-file-share-2.png)
1. **+ 파일 공유**를 선택합니다.
1. 이름 아래에 **마케팅을 입력한 다음, 다음: 백업**을 선택합니다**.**
   ![그래픽 사용자 인터페이스, 애플리케이션 자동 생성된 설명](../media/new-file-share-basics.png)
1. 다음 그림과 같이 백업** 사용 선택 취소**: ![그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/new-file-share-backup.png)
1. **검토 + 만들기**를 선택합니다. 리소스의 유효성을 검사한후 **만들기**를 선택합니다.
...
1. **선택한 가상 네트워크 및 IP 주소에서 사용**을 선택합니다.

1. **+ 기존 가상 네트워크 추가**를 선택합니다.

1. 네트워크** 추가에서 **다음 값을 ![선택합니다. 그래픽 사용자 인터페이스, 애플리케이션 설명이 자동으로 생성됨](../media/add-network-access.png)
...
1. Cloud Shell 창의 도구 모음에서 파일** 업로드/다운로드 아이콘을 선택하고**, 드롭다운 메뉴에서 업로드**를 선택하고 **다음 파일 **VMs.json** 및 **VMs.parameters.json**을 원본 폴더 **F:\Allfiles\Exercises\M07**에서 하나씩 Cloud Shell 홈 디렉터리에 업로드합니다.
...
1. VM이 명령 프롬프트에서 인터넷에 대한 아웃바운드 연결이 없는지 확인합니다.
...
> **경고**: 계속하기 전에 이 랩에 사용되는 모든 리소스를 제거해야 합니다. 이렇게 하려면 Azure Portal에서 리소스 그룹을 선택합니다. 만든 리소스 그룹을 선택합니다. 리소스 그룹 창에서 리소스 그룹 삭제를 선택하고 리소스 그룹 이름을 입력한 후 삭제를 선택합니다. 만들었을 수 있는 추가 리소스 그룹에 대해 이 프로세스를 반복합니다. 이렇게 하지 않으면 다른 랩에서 문제가 발생할 수 있습니다.
