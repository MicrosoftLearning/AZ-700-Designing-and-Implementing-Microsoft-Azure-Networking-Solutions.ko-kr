...
# M04-Unit 4 Azure 부하 분산 장치 만들기 및 구성
...
6. IP 주소** 탭의 **IPv4 주소 공간** 상자에서 **기본값을 제거하고 10.1.0.0/16**을 입력**합니다.
...
1. 페이지 맨 위에 있는 검색 상자에서 Load Balancer**를 입력**한 다음 Enter 키를 누릅니**다**(**참고:** 목록에서 선택하지 않음).
...
1. 설정 상태 **프로브를** 선택한 **다음 추가**를 선택합니다**.**
...
1. Azure Portal 홈페이지에서 리소스** 만들기, 가상** 머신을 선택한 **다음**, 가상 머신**을 선택합니다**(이 리소스 종류가 페이지에 나열되지 않은 경우 페이지 맨 위에 있는 검색 상자를 사용하여 해당 리소스를 검색하고 선택합니다).
...
1. **사용자 이름** 상자에 TestUser**를 입력**하고 **암호** 상자에 만든 암호를 입력한 다음, 커넥트** 선택합니다**. 팝업 차단기가 새 창을 차단하는 경우 팝업 차단기를 허용하고 다시 **연결**합니다.
